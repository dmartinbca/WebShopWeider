    ...
    public class RestDataService
    {
        ...
        public async Task<NavUser> GetAppLoginAPICloud(string usuario, string password)
        {
            ...
            try
            {
                ...
                var tokenResponse = await _http.PostAsJsonAsync("https://tentaloatenticator-f9gvcxh9f9evcmbq.canadacentral-01.azurewebsites.net/api/token", request1);
        ...
        public async Task<List<Stock>> GetStockProductoAsync(string almacen, string producto)
        {
            ...
            try
            {
                ...
                var tokenResponse = await _http.PostAsJsonAsync("https://tentaloatenticator-f9gvcxh9f9evcmbq.canadacentral-01.azurewebsites.net/api/token", requestToken);
                if (tokenResponse.IsSuccessStatusCode)
                {
                    var tokenObj = await tokenResponse.Content.ReadFromJsonAsync<TokenResponse>();
                    token = tokenObj.AccessToken;
        ...
        public async Task<List<PromoHeader>> GetPromocionesAPICloud(string customerNo = "")
        {
            ...
            try
            {
                ...
                var tokenResponse = await _http.PostAsJsonAsync(
                    "https://tentaloatenticator-f9gvcxh9f9evcmbq.canadacentral-01.azurewebsites.net/api/token",
                    requestToken);
